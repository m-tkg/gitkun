...
enum KuntraykunMenuExport {
    ...
    /// gitkun のメニューは動的構築（`menuNeedsUpdate`）だが、`menu.update()` が
    /// delegate の `menuNeedsUpdate` を同期的に呼ぶため、ここで最新内容に再構築されてから読める。
    static func export(_ menu: NSMenu) {
        guard let fileURL else { return }
        menu.update() // enabled 状態を確定させてから読む（動的メニューは最新内容に再構築される）。
    ...
    private static func nodes(of menu: NSMenu, path: [Int]) -> [Node] {
        ...
        for (index, item) in menu.items.enumerated() {
            ...
            // カスタムビュー項目（NotificationMenuItemView の行）はビューを転送できないため、
            // タイトルのみ・操作不可で書き出す（クリックはビュー側が処理しており action が無い）。
            if item.view != nil {
